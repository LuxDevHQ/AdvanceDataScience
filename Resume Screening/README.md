...
The point of this step is **feature selection**: test every remaining feature (after Step 3's drops) against the target so you know, with evidence rather than a guess, which ones are actually worth keeping.

- **Numerical feature vs. target** (e.g. `interview_score`, `technical_test_score`, `ats_score`, `experience_years`, `keyword_match_percentage`, `cgpa`, `age`, `projects_completed`, `publications`, `expected_salary`, `resume_quality_score`, `resume_length`, `communication_score`, `problem_solving_score`, `aptitude_score`, your engineered count features, and any other numeric column): since `selected` has exactly 2 groups, use an independent t-test on each one to check if its average really differs between `Selected` and `Rejected`. State H₀, H₁, and your conclusion from the p-value.
- **Categorical feature vs. target** (e.g. `gender`, `highest_education`, `university`, `field_of_study`, `internship_experience`, `leadership_experience`, `job_role`, `employment_type`, `remote_preference`, `availability`, `country`, and any other categorical column you kept): use a Chi-square test on each one to check if the feature and target are related.
...
**Hypothesis test results:** Which features were statistically significant against `selected` (and therefore kept), and which were dropped.
